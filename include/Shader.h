...
public:
    /**
     * Destructor frees internal shader.
     */
    Shader::~Shader() { mShader.free(); }
     ...
     */
    bool Shader::initFromFiles(const std::string& name,
        const std::string vertex_fnames[], const std::string fragment_fnames[],
        const std::string geometry_fnames[], int numVert, int numFrag,
        int numGeom)
    {
        std::string vName = "";
        std::string fName = "";
     ...
     */
    void Shader::drawArraysInstanced(
        int type, uint32_t offset, uint32_t vertices, uint32_t count)
    {
        if (count == 0) return;
     ...
     */
    void Shader::drawElementsInstanced(
        int type, uint32_t offset_, uint32_t vertices, uint32_t count_)
    {
        if (count_ == 0) return;
    ...
    void Shader::setUniform(
        const std::string& name, const nanogui::GLUniformBuffer& buf, bool warn)
    {
    ...
    void Shader::shareAttrib(const GLShader& otherShader,
        const std::string& name, const std::string& _as)
    {
        mShader.shareAttrib(otherShader, name, _as);
    }

    void Shader::invalidateAttribs() { mShader.invalidateAttribs(); }
    ...
    void Shader::free() { mShader.free(); }

    const nanogui::GLShader::Buffer& Shader::attribBuffer(
    ...
    void Shader::uploadAttrib(const std::string& name, size_t size, int dim,
        uint32_t compSize, GLuint glType, bool integral, const void* data,
        int version)
    {
        mShader.uploadAttrib(
            name, size, dim, compSize, glType, integral, data, version);
    }

    void Shader::downloadAttrib(
        const std::string& name, size_t size, uint32_t compSize, void* data)
    {
        mShader.downloadAttrib(name, size, 0, compSize, 0, data);
    }

    const std::string& name() const { return mShader.name(); }
    ...
    void uploadAttrib(
        const std::string& name, const Matrix& M, int version = -1)
    {
    ...
    void setUniform(const std::string&   name,
        const Eigen::Transform<T, 3, 3>& affine, bool warn = true)
    {
        mShader.setUniform(name, mat, warn);
    }

    template<typename T>
    void setUniform(const std::string& name, const Eigen::Matrix<T, 3, 3>& mat,
        bool warn = true)
    {
        mShader.setUniform(name, mat, warn);
    }
    ...
    template<typename T>
    void setUniform(const std::string&   name,
        const Eigen::Transform<T, 2, 2>& affine, bool warn = true)
    {
    ...
    template<typename T,
        typename std::enable_if<nanogui::detail::type_traits<T>::integral == 1,
            int>::type = 0>
    ...
    template<typename T,
        typename std::enable_if<nanogui::detail::type_traits<T>::integral == 0,
            int>::type = 0>
    void setUniform(const std::string& name, T value, bool warn = true)
    {
        mShader.setUniform(name, value, warn);
